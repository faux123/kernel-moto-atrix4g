...
static bool is_cpcap_wake_reason( void );
...
struct cpcap_spi_init_data tegra_cpcap_spi_init[] = {
	...
	{CPCAP_REG_S5C,       0x0020},   /* Set SW5 to On/Off. */
...
unsigned short cpcap_regulator_mode_values[CPCAP_NUM_REGULATORS] = {
	...
	[CPCAP_SW5]      = 0x0020,  /* On/Off Updated below for products which support macro control. */
...
static struct regulator_init_data cpcap_regulator[CPCAP_NUM_REGULATORS] = {
	...
	[CPCAP_VWLAN2] = {
		.constraints = {
			...
			.always_on		= 1,  /* For P3 this is no longer always on, see the code
									 in mot_setup_power() */
		},
...
struct cpcap_platform_data tegra_cpcap_data = 
{
	...
	.wdt_disable = 0,
	.is_cpcap_wake_reason = is_cpcap_wake_reason,
	.hwcfg = {
...
static struct fixed_voltage_config fixed_sdio_config = {
	...
	.gpio = TEGRA_GPIO_PF3,
	.enable_high = 1,
	.enabled_at_boot = 1,		/* Needs to be enabled on all hardware prior to P3
								   P3 is handled below. */
	...
	if (machine_is_olympus()) {
		if (HWREV_TYPE_IS_FINAL(system_rev) ||
			...
			if (tegra_cpcap_spi_init[CPCAP_REG_WLAN2_INDEX].reg == CPCAP_REG_VWLAN2C) {
				tegra_cpcap_spi_init[CPCAP_REG_WLAN2_INDEX].data =  0x0089; /* Set VWLAN2 to On/LP 3.3v. */
			}
			else {
			...
			cpcap_regulator_mode_values[CPCAP_VWLAN2] = 0x0009; /* On/LP 3.3v Secondary Standby */
			cpcap_regulator_off_mode_values[CPCAP_VWLAN2] = 0x0004; /* Turn off only once sec standby is entered. */
		...
		/* All versions of olympus support shutting down SW5 when binking the message LED.
		   Set SW5 to On/Off Secondary control when off.
		   Indicate the macro controls SW5. */
		cpcap_regulator_mode_values[CPCAP_SW5] = CPCAP_REG_OFF_MODE_SEC | 0x0020;
...
#define WAKE_REASON_CPCAP     0x00040000
static bool is_cpcap_wake_reason( void )
{
	return ((readl(IO_ADDRESS(WAKE_REASON_ADDR)) & WAKE_REASON_CPCAP)?true:false);
}
