...
static const NvOdmSdramControllerConfigAdv s_NvOdmHynix512M50nmEmcConfigTable[] =
{
    {
                  0x20,   /* Rev 2.0 */
...
NvOdmQuerySdramControllerConfigGet(NvU32 *pEntries, NvU32 *pRevision)
{
        ...
        if (pRevision)
            *pRevision = s_NvOdmElpida512M50nmEmcConfigTable[0].Revision;
        if (pEntries)
            *pEntries = NV_ARRAY_SIZE(s_NvOdmElpida512M50nmEmcConfigTable);
        return (const void*)s_NvOdmElpida512M50nmEmcConfigTable;
#endif
    if (pEntries)
        *pEntries = 0;
    return NULL;
