...
static const NvOdmSdramControllerConfigAdv s_NvOdmHynix512MEmcConfigTable[] =
{
    {
                  0x20,   /* Rev 2.0 */
                    ...
                    46,   /* Number of EMC parameters below */
        {
            ...
            0x004B030E,   /* CFG_DIG_DLL */
            0x007f9810,   /* DLL_XFORM_DQS */
...
static const NvOdmSdramControllerConfigAdv s_NvOdmElpida1GEmcConfigTable[] =
{
    {
                  0x20,   /* Rev 2.0 */
                 18000,   /* SDRAM frquency */
...
NvOdmQuerySdramControllerConfigGet(NvU32 *pEntries, NvU32 *pRevision)
{
    ...
    if (HWREV_TYPE_IS_FINAL(system_rev) ||
        (machine_is_olympus() && HWREV_TYPE_IS_PORTABLE(system_rev)))
    {
	...
	if (NvRmOpen (&pDev, 0) != NvError_Success)
        {
            printk("%s: NvRmOpen() error\n",__func__);
        }
        // read Manufacturer ID
        NV_REGR(pDev, NvRmPrivModuleID_ExternalMemoryController, 0, EMC_MRR_0); // dummy read
        while(NV_REGR(pDev, NvRmPrivModuleID_ExternalMemoryController, 0,
                       EMC_EMC_STATUS_0) & EMC_EMC_STATUS_0_MRR_DIVLD_FIELD);
        NV_REGW(pDev, NvRmPrivModuleID_ExternalMemoryController, 0, EMC_MRR_0, (1<<30)|(5<<16));

        while (((NV_REGR(pDev, NvRmPrivModuleID_ExternalMemoryController, 0,
                          EMC_EMC_STATUS_0) >> EMC_EMC_STATUS_0_MRR_DIVLD_SHIFT) & 0x1) != 1);
        idreg = NV_REGR(pDev, NvRmPrivModuleID_ExternalMemoryController, 0, EMC_MRR_0);

        switch (idreg & MAN_ID_MASK)
        {
           case ELPIDA:
              if (pRevision)
                  *pRevision = s_NvOdmElpida1GEmcConfigTable[0].Revision;
              if (pEntries)
                  *pEntries = NV_ARRAY_SIZE(s_NvOdmElpida1GEmcConfigTable);
              return (const void*)s_NvOdmElpida1GEmcConfigTable;
           case MICRON:
              if (pRevision)
                  *pRevision = s_NvOdmMicron1GEmcConfigTable[0].Revision;
              if (pEntries)
                  *pEntries = NV_ARRAY_SIZE(s_NvOdmMicron1GEmcConfigTable);
              return (const void*)s_NvOdmMicron1GEmcConfigTable;
           default:
              printk("%s: unknown LPDDR vendor\n",__func__);
        }
    }
    else
    {
        if (pRevision)
            *pRevision = s_NvOdmHynix512MEmcConfigTable[0].Revision;
        if (pEntries)
            *pEntries = NV_ARRAY_SIZE(s_NvOdmHynix512MEmcConfigTable);
        return (const void*)s_NvOdmHynix512MEmcConfigTable;
    }
#endif
    if (pEntries)
        *pEntries = 0;
