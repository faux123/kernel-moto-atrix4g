...
/*
 * bl_ver_* contain the bootloader version.
 * uboot_ver_* contain the microboot version.
 * bl_ver_* and uboot_ver_* default to 0 if they are
 * not set.
 *
 * Exported symbols:
 * bi_bl_version()                -- returns the MBM version
 * bi_set_bl_version()            -- sets the MBM version
 * bi_uboot_version()             -- returns the MBM loader version
 * bi_set_uboot_version()         -- sets the MBM loader version
 */
extern u8 bootloader_ver_major;
extern u8 bootloader_ver_minor;
u32 bi_bl_version(void)
{
	return (bootloader_ver_major << 8) | (bootloader_ver_minor);
}
EXPORT_SYMBOL(bi_bl_version);

void bi_set_bl_version(u32 __bl_version)
{
	bootloader_ver_major = (__bl_version & 0xFF00) >> 8;
	bootloader_ver_minor = __bl_version & 0x00FF;
}
EXPORT_SYMBOL(bi_set_bl_version);

extern u8 uboot_ver_major;
extern u8 uboot_ver_minor;
u32 bi_uboot_version(void)
{
	return (uboot_ver_major << 8) | (uboot_ver_minor);
}
EXPORT_SYMBOL(bi_uboot_version);

void bi_set_uboot_version(u32 __uboot_version)
{
	uboot_ver_major = (__uboot_version & 0xFF00) >> 8;
	uboot_ver_minor = __uboot_version & 0x00FF;
}
EXPORT_SYMBOL(bi_set_uboot_version);

#define EMIT_BL_VERSION() \
	    EMIT_BOOTINFO("BL_VERSION", "0x%08x", bl_version)
#define EMIT_UBOOT_VERSION() \
	    EMIT_BOOTINFO("UBOOT_VERSION", "0x%08x", uboot_version)
