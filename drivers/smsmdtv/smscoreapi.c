...
extern u8 *g_p_fw_buf;

// to enable log, set to 3, disable: 0
int sms_debug = 0;
...
static struct smscore_registry_entry_t *smscore_find_registry(char *devpath)
{
	...
	if (entry) {
		...
		{
			//eladr
			sms_err("eladr: strlen(devpath) >= 32\n");
			return NULL;
		}
...
// for loopback test
	...
	for (buffer = dev->common_buffer; 
		...
		struct smscore_buffer_t *cb = smscore_createbuffer(buffer, dev->common_buffer, dev->common_buffer_phys);
		if (!cb) {
			smscore_unregister_device(dev);
			return -ENOMEM;
		}
		...
		void *buffer, size_t size, struct completion *completion) 
{
	...
	return wait_for_completion_timeout(completion, msecs_to_jiffies(4000)) ? 0 : -ETIME;
}
/**
 ...
 * @return 0 on success, <0 on error.
 */
static int smscore_load_firmware_family2(struct smscore_device_t *coredev, void *buffer, size_t size) 
{

	u8 *msg_buf;
	u8 *data_position = g_p_fw_buf;		// pointer to user space fw buffer
	struct SmsFirmware_ST *firmware;
	struct SmsMsgHdr_ST *msg;
	u32 mem_address, trigger_addr;
	int rc = 0;
	size_t left_length, left_data;

	sms_info("fw addr(user space): 0x%p, whole fw size %d", data_position, (int)size);

	msg_buf = kmalloc(PAGE_SIZE, GFP_KERNEL | GFP_DMA);
	if (!msg_buf)
	{
		sms_err("failed to allocate memory for msg buffer");
		return -ENOMEM;		
	}	

	if (copy_from_user(msg_buf, data_position, PAGE_SIZE)) 
	{
		sms_err("failed to copy fw content from user buffer");
		kfree(msg_buf);
	        msg_buf = NULL;
		return -EFAULT;
	}

	firmware = (struct SmsFirmware_ST *) msg_buf;
	firmware->Length = le32_to_cpu(firmware->Length);
	firmware->StartAddress = le32_to_cpu(firmware->StartAddress);
	u8 *payload = firmware->Payload;
	mem_address = firmware->StartAddress;
	trigger_addr = firmware->StartAddress;

	sms_info("Loading FW data to addr 0x%x size %d", mem_address, firmware->Length);
	//sms_info("payload addr 0x%x, buf addr 0x%x, sizeof(SmsFirmware_ST): %d", payload, msg_buf, sizeof(struct SmsFirmware_ST));
	...
	//============================== transfer first PAGE_SIZE(4k) block fw data =======================================
	left_data = PAGE_SIZE - sizeof(struct SmsFirmware_ST) + 4;
	while (left_data && rc >= 0) {
		struct SmsDataDownload_ST *DataMsg = (struct SmsDataDownload_ST *) msg;
		int payload_size = min((int)left_data, SMS_MAX_PAYLOAD_SIZE);
		//sms_info("payload addr: 0x%x, payload size: %d, mem addr: 0x%x", payload, payload_size, mem_address);
		...
		//sms_info("payload contents: 0x%x,0x%x,0x%x,0x%x", payload[0], payload[1], payload[2], payload[3]);
		...
		if(rc != 0)
		{
			printk("send 660 res: %d\n", rc);
		}

		payload += payload_size;
		left_data -= payload_size;
		mem_address += payload_size;
	}

	//============================== transfer left blocks fw data =======================================
	left_length = size - PAGE_SIZE;
	data_position += PAGE_SIZE;
 
	while(left_length > 0 && rc >= 0){
	
		int copy_size = min((int)left_length, PAGE_SIZE);
		if (copy_from_user(msg_buf, data_position, copy_size)) 
		{
			sms_err("failed to copy fw content from user buffer");
			kfree(msg_buf);
	     	   	msg_buf = NULL;
		...
		payload = msg_buf;   // change data pointer to buffer start position
		left_data = copy_size;
		//sms_info("copyload size: %d, payload addr 0x%x, fw mem addr 0x%x", copy_size, payload, mem_address);
	...
	if (rc >= 0) {
		if (coredev->mode == DEVICE_MODE_NONE) {
			...
			TriggerMsg->msgData[0] = trigger_addr; /* Entry point */
			...
			if (coredev->device_flags & SMS_ROM_NO_RESPONSE) {
				rc = coredev->sendrequest_handler(coredev->context, TriggerMsg,
	...
	return ((rc >= 0) && coredev->postload_handler) ? coredev->postload_handler(coredev->context) : rc;
}
...
/*DVBT*/
{ "none", "dvb_nova_12mhz.inp", "dvb_nova_12mhz_b0.inp", "none", "none", "none"},
/*DVBH*/
{ "none", "dvb_nova_12mhz.inp", "dvb_nova_12mhz_b0.inp", "none", "none", "none" },
/*TDMB*/
{ "none", "tdmb_nova_12mhz.inp", "tdmb_nova_12mhz_b0.inp", "none", "none", "none" },
/*DABIP*/
...
{ "none", "isdbt_nova_12mhz.inp", "isdbt_nova_12mhz_b0.inp", "none", "none", "none" },
/*CMMB*/
{ "none", "none", "none", "cmmb_vega_12mhz.inp", "cmmb_venice_12mhz.inp", "cmmb_ming_app.inp" },
/*RAW - not supported*/
...

{
	...
	coredev->mode = DEVICE_MODE_NONE;	
    	sms_debug("reset device mode to default %d", coredev->mode);
...
int smscore_set_device_mode(struct smscore_device_t *coredev, int mode)
{
	...
	if (rc >= 0) {
		sms_debug("set device mode to %d successfully", mode);
		coredev->mode = mode;
...
void smscore_onresponse(struct smscore_device_t *coredev,
	...
	if (time_now - last_sample_time > 10000) {
		sms_debug("\n\tdata rate %d bytes/secs", (int)((data_total * 1000)/(time_now - last_sample_time)));
		last_sample_time = time_now;
		data_total = 0;
	}
        
sms_info("Msg type = %d, len = %d", phdr->msgType, phdr->msgLength); 
	data_total += cb->size;
...
struct smscore_buffer_t *smscore_getbuffer(struct smscore_device_t *coredev)
{
	...
	if (list_empty(&coredev->buffers))
	{
		...
		//eladr - to avoid rx buffers hung
...
int smscore_gpio_configure(struct smscore_device_t *coredev, u8 PinNum,
	...
	rc = smscore_sendrequest_and_wait(coredev, pMsg, totalLen, &coredev->gpio_configuration_done);
...
static int __init smscore_module_init(void)
{
	 ...
	 */
#ifdef SMS_SPI_NVIDIA_AP20_DRV
	/* NVIDIA AP20 SPI Register */
	rc = smsspi_register();
	sms_debug("smsspi registered, %d", rc);
	if (rc) {
		sms_err("Error registering NVIDIA AP20 SPI bus driver.\n");
		goto smsspi_error;
	}
#endif
...
MODULE_DESCRIPTION("Siano MDTV Core module - Version 2.2.2");
