	...
	u8 *buf, int len, int type)
{
	...
	mux_list = kmalloc(sizeof(struct mux_logger_list_t), GFP_KERNEL);
	if (!mux_list) {
	...
	if (buf[0] == TS0710_BASIC_FLAG)
		...
		offset = 0;
	mux_list->body = kmalloc(len + offset, GFP_KERNEL);
	if (!(mux_list->body)) {
		kfree(mux_list);
		mux_print(MSG_ERROR, "can not malloc (%d) memory!\n", len);
		return NULL;
...
struct ts27010_mux_logger *ts27010_alloc_mux_usb_logger(void)
{
	...
	mux_logger = kmalloc(sizeof(struct ts27010_mux_logger), GFP_KERNEL);
	if (mux_logger == NULL) {
